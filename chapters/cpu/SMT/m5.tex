...
different instance types belonging to this family. 
\begin{table}[H]
\centering
\begin{tabular}{lcc}
\hline
...
\end{table}
\noindent
The m5 dedicated host used the 2nd-generation Intel CPU in all our benchmarks. 
We repeated the experiments using hosts running on the 1st-generation CPU, and we received the same
...
\end{axis}
\end{tikzpicture}
\caption{Effect of adding busy neighbors on the CPU speed of the test node (m5.2xlarge)}
...
\end{axis}
\end{tikzpicture}
\caption{Effect of adding idle neighbors on the CPU speed of the test node (m5.2xlarge)} 
...
This is mainly caused by the physical core co-location that happens between the different 
threads, resulting in resource contention, as the execution resources are not duplicated. 
In all our previous experiments, the instances 
...
degradation compared to the smaller instances (large and xlarge). The reason is that the 
...
a performance degradation of 27\%. With this finding, H2 is rejected since co-tenants can not share
physical cores. \\
\begin{comment}
